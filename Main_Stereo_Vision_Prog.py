...
def coords_mouse_disp(event,x,y,flags,param):
    if event == cv2.EVENT_LBUTTONDBLCLK:
        ...
        print('Distance: '+ str(Distance*0.384)+' m')
...
if int(input("Need to calibrate (1/0 - yes/no): ")) == 1:
    # Start calibration from the camera
    print('Starting calibration for the 2 cameras... ')
    # Call all saved images
    for i in range(0,96):   # Put the amount of pictures you have taken for the calibration inbetween range(0,?) wenn starting from the image number 0
        t= str(i)
        ChessImaR= cv2.imread('img/right'+t+'.png',0)    # Right side
        ChessImaL= cv2.imread('img/left'+t+'.png',0)    # Left side
...
window_size = 3
min_disp = 2 #2
num_disp = 130-min_disp #160
stereo = cv2.StereoSGBM_create(minDisparity = min_disp,
    numDisparities = num_disp,
    blockSize = window_size,
    uniquenessRatio = 10, #15,
    speckleWindowSize = 100, #0,
    speckleRange = 32, #2,
    disp12MaxDiff = 5, #5,
    P1 = 8*3*window_size**2,
    P2 = 32*3*window_size**2,       
    #preFilterCap=63, #none
    #fullDB = True,
...
##    dispR= cv2.resize(disp,None,fx=0.7, fy=0.7, interpolation = cv2.INTER_AREA)
    ...
    cv2.imshow('Filtered Color Depth',closing)

    # Mouse click
    cv2.setMouseCallback("Filtered Color Depth",coords_mouse_disp,closing)
